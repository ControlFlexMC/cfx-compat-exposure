...
> This branch (`1.20.1`) builds the two 1.20.1 loaders; the MC 1.21.1 flavors (Fabric + NeoForge) live on the `1.21.1` branch. Exposure 1.9.21 has no 1.20.1 Fabric release, so the Fabric side compiles against 1.9.20 (the latest of that platform); at runtime both loaders accept Exposure ≥ 1.9.20.

Both loaders share the same loader-agnostic Java sources (`common/src/main/java`); platform differences live only in build scripts and metadata (mods.toml / fabric.mod.json, mixins.json `compatibilityLevel`, pack.mcmeta `pack_format`).
...
- Runtime metadata: both loaders require `controlflex [0.8.7,)` and `exposure [1.9.20,)` — both required, CLIENT.
...
- `./gradlew :forge:build :fabric:build` produces both JARs.
