#!/usr/bin/env bash
# Build the Fabric (MC 1.21.1) artifact for cfx-compat-exposure.
...
#
# Environment:
#   MC_1_21_1_FABRIC_PATH  instance dir; unset → skip instance deploy (WARNING)
...
# ── Artifact ──────────────────────────────────────────────────────────────────
NEW_JAR="${PROJECT_DIR}/fabric/build/libs/cfx-compat-exposure-${label}-mc${mc_ver}-fabric.jar"
if [[ ! -f "${NEW_JAR}" ]]; then
...
# ── Instance deploy ───────────────────────────────────────────────────────────
MC_INSTANCE="${MC_1_21_1_FABRIC_PATH:-}"
MODS_DIR="${MC_INSTANCE}/mods"

if [[ -z "${MC_INSTANCE}" ]]; then
    echo "⚠️  MC_1_21_1_FABRIC_PATH is not set; skipping instance deploy"
    ...
    if [[ -d "${MODS_DIR}" ]]; then
        rm -f "${MODS_DIR}"/cfx-compat-exposure-*-mc*-fabric.jar
        echo "  removed old cfx-compat-exposure Fabric jars"
    else
