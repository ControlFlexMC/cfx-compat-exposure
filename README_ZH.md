...
> 本分支（`1.21.1`）面向 **Minecraft 1.21.1（Fabric + NeoForge）**；Minecraft 1.20.1 版本（Forge + Fabric）见 `1.20.1` 分支。
...
- **Exposure** ≥ 1.9.18（Minecraft 1.21.1，Fabric 与 NeoForge 21.1+）
- 仅客户端。
...
```bash
./tools/build-fabric.sh          # MC 1.21.1 Fabric   → fabric/build/libs/
./tools/build-neoforge.sh        # MC 1.21.1 NeoForge → neoforge/build/libs/
# 或：./gradlew :fabric:build :neoforge:build
```

依赖说明：ControlFlex API 经 JitPack 解析（`com.github.ControlFlexMC:control-flex-api:0.8.7`，纯 Java、与 loader 无关）；Exposure 走 CurseForge Maven（`curse.maven:exposure-871755:*`，Fabric 文件 8223556 / NeoForge 文件 8223555 —— 均为 1.21.1 的 Exposure 1.9.18），与 cfx-compat-epicfight 对 Epic Fight 的依赖方式一致。
